...
{{ formula }}-devtools-{{ tool }}-alternatives-install-bin-{{ cmd }}:
  ...
  alternatives.install:
    ...
    - priority: {{ d.linux.altpriority }}
                        {%- else %}
  cmd.run:
    - name: update-alternatives --install /usr/local/{{ cmd }} link-k8s-devtools-{{ tool }}-{{ cmd }} {{ d.devtools['pkg'][tool]['path'] }}/{{ cmd }} {{ d.linux.altpriority }} # noqa 204
                        {%- endif %}

    - onlyif: test -f {{ d.devtools['pkg'][tool]['path'] }}/{{ cmd }}
    - unless: update-alternatives --get-selections |grep ^link-k8s-devtools-{{ tool }}-{{ cmd }}
    - require:
