...
k8s-kubectl-config-alternatives-install-k8s-kubectl-alternative-install:
  cmd.run:
    ...
    - name: update-alternatives --install /usr/local/bin/kubectl link-k8s-kubectl {{ k8s.dir.binary }}/{{ k8s.kubectl.pkg.binary.name }}/bin/kubectl {{ k8s.kubectl.linux.altpriority }}
