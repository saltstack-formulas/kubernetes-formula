...
We will see a quick setup to configure the ``kubernetes`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``kubernetes`` formula
```````````````````````````````````````````````````````

Now, we can provides the per DNS domain name configuration files for the ``kubernetes`` formulas under ``/srv/salt/kubernetes/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/kubernetes/parameters/dns:domain/
    ln -s dns:domain /srv/salt/kubernetes/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/kubernetes/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/kubernetes/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``kubernetes`` formula
````````````````````````````````````````````````````````

Now, we can provides the per role configuration files for the ``kubernetes`` formulas under ``/srv/salt/kubernetes/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/kubernetes/parameters/roles
...
We create a configuration for the role ``kubernetes/server`` in ``/srv/salt/kubernetes/parameters/roles/kubernetes/server.yaml``:
...
We create another configuration for the role ``kubernetes/client`` in ``/srv/salt/kubernetes/parameters/roles/kubernetes/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles kubernetes/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles kubernetes/client
...
.. code-block:: console
    ...
    mv /srv/salt/kubernetes/parameters/dns:domain/  '/srv/salt/kubernetes/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    {%- from tplroot ~ "/map.jinja" import mapdata as kubernetes with context %}

    test-does-nothing-but-display-kubernetes-as-json:
      test.nop:
...
Here is an example based on `template-formula/kubernetes/config/file.sls`_:
...
.. code-block:: sls
    ...
    kubernetes-config-file-file-managed:
      file.managed:
        - name: {{ kubernetes.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='kubernetes-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
        - group: {{ kubernetes.rootgroup }}
        - makedirs: True
        - template: jinja
        - require:
...
This ``sls`` file expose a ``kubernetes`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ kubernetes['winner'] }}
...
.. _template-formula/kubernetes/config/file.sls: https://github.com/saltstack-formulas/template-formula/blob/master/kubernetes/config/file.sls
